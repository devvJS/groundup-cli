...
function buildPhasePrompt(phase, blueprint, workflow) {
...
export async function runBuild({ projectRoot }) {
  ...
  // --- Phase 1 execution (single phase for this commit) ---
  ...
  const promptContent = buildPhasePrompt(phase, blueprint, workflow);
  ...
  const choice = await askSelect(
    ...
    [
      ...
      { value: 'retry', label: 'Retry — run this phase again' },
      { value: 'abort', label: 'Abort — stop the build' },
    ],
    'approve'
  );
  ...
  if (choice === 'approve') {
    ...
    sep();
    line();
    return { completed: true, phase: phase.number };
  }

  if (choice === 'retry') {
    line();
    console.log(muted('  Retrying phase — run groundup build again.'));
    line();
  ...
  // abort
  line();
  console.log(muted('  Build aborted.'));
  line();
  return { completed: false };
}
